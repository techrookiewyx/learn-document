...
通过给元素设置浮动可以让它向其父元素的左侧或右侧移动，以达到元素水平排列，当给元素设置浮动以后，就不必遵循过度约束的等式，而且设置了浮动的元素会脱离文档流不再占用文档流中文位置
...
- 脱离文档流的元素块不再独占一行，当没有指定宽度高度时，其高度和宽度被内容撑开
- 脱离文档流的行内元素，其特点会和脱离文档流的块元素一样

### 高度塌陷

当我们希望一个父元素会根据其子元素内容决定高度时，子元素浮动后，他会脱离文档流，而其父元素没有高度的情况下，子元素无法撑起父元素，导致父元素高度丢失，从而页面中其他元素的布局，下面介绍其解决方法
